...
namespace Experiment.NoobAutoLinker.Editor
{
    public class AutoLinkerEditor : EditorWindow
    {
        ...
        private void ReInjectToObject(Object objectToLink)
        {
            var linkResults = _autoLinker.InjectToObject(objectToLink, shouldOverrideExistingValue).ToArray();
            foreach (var linkResult in linkResults)
                HandleLinkResult(linkResult);
        ...
        private void InjectToScene(Scene scene)
        {
            var linkResults = _autoLinker.InjectToScene(scene, shouldOverrideExistingValue).ToArray();

            foreach (var linkResult in linkResults) HandleLinkResult(linkResult);
